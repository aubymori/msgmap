...
`;
    ...
    {
        fileText +=
`   const ${charType} *(*${strName})(${formatCArgs(defaultLangData.formattedStrings[strName])});
`;
    }
...
`;
    ...
    for (const strName in langData.formattedStrings)
    {
        ...
        fileText +=
`inline const ${charType} *mm_${record}_translations_${strName}_${lang}(${formatCArgs(defaultLangData.formattedStrings[strName])})
{
    static const ${charType} *format = ${formatCString(strData.text)};
    size_t length = mm_snprintf(NULL, 0, format, ${formatCCallArgs(strData)});
    ${charType} *buffer = (${charType} *)malloc((length + 1) * sizeof(${charType}));
    if (!buffer)
        return NULL;
    mm_snprintf(buffer, length, format, ${formatCCallArgs(strData)});
    return buffer;
}
